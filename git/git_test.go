...
func TestParseFilesRenameStatus(t *testing.T) {
	// Rename lines from git diff --name-status have three tab-separated fields.
	// We only care about the status char and the destination path (second field).
	...
	if got[0].Status != "R" {
		t.Errorf("expected status R, got %q", got[0].Status)
	}
	if got[0].Path != "old.go" {
		t.Errorf("expected path old.go, got %q", got[0].Path)
	}
}
