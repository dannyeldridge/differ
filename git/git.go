...
// LoadStagedFiles returns files with staged changes (git diff --cached).
func LoadStagedFiles(repoPath string) ([]FileChange, error) {
	out, err := run(repoPath, "diff", "--cached", "--name-status")
	if err != nil {
		return nil, err
	}
	return parseFiles(out), nil
...
func parseFiles(output string) []FileChange {
	...
	for _, line := range lines {
		...
		// For renames/copies (3 fields), use the source path (old.go).
		// For regular changes (2 fields), use parts[1].
		path := parts[1]
